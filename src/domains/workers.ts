...
export async function getNodeWorkerRuns(
  ...
  const res = await fetch(
    `${internals(agent).platformUrl}/api/nodes/me/worker-runs?${params.toString()}`,
    {
...
export async function createWorkerRun(
  agent: AgentPactAgent,
  input: WorkerRunCreateInput,
): Promise<WorkerRunData> {
  const res = await fetch(
    `${internals(agent).platformUrl}/api/nodes/me/worker-runs`,
    {
      method: "POST",
      headers: internals(agent).headers(),
...
export async function startWorkerTaskSession(
  agent: AgentPactAgent,
  ...
  const run = await agent.createWorkerRun({
    ...
    summary:
      input.summary ??
      `Execution session started for ${task.title ?? input.taskId}`,
...
export async function resumeWorkerTaskSession(
  agent: AgentPactAgent,
  ...
  const existingRun = runs
    ...
    .sort((a, b) => {
      const aTime = new Date(
        String(a.lastHeartbeatAt ?? a.updatedAt ?? a.createdAt ?? 0),
      ).getTime();
      const bTime = new Date(
        String(b.lastHeartbeatAt ?? b.updatedAt ?? b.createdAt ?? 0),
      ).getTime();
      return bTime - aTime;
    })[0];
  ...
  const run = await agent.heartbeatWorkerRun(existingRun.id, {
    ...
    currentStep:
      input.currentStep ?? existingRun.currentStep ?? "Worker session resumed",
    summary:
      input.summary ??
      existingRun.summary ??
      `Execution session resumed for ${task.title ?? input.taskId}`,
    metadata: input.metadata,
  });
...
export async function claimTaskForWorkerRun(
  agent: AgentPactAgent,
  ...
  const [run, task] = await Promise.all([
    agent.updateWorkerRun(input.runId, {
      ...
      currentStep:
        input.currentStep ??
        "Task claimed on-chain, protected execution unlocked",
      summary:
        input.summary ??
        "Task claimed successfully and worker execution may continue.",
      metadata: input.metadata,
    }),
    agent.fetchTaskDetails(input.taskId),
  ]);
...
export async function getWorkerTaskExecutionBrief(
  agent: AgentPactAgent,
  ...
  if (
    task.access?.assignmentRole !== "selected_provider" &&
    task.access?.assignmentRole !== "claimed_provider"
  ) {
    suggestedNextActions.push(
      "Verify the current node is the assigned provider before executing protected task work.",
    );
  }
  if (task.workflow?.canSelectedNodeClaim) {
    suggestedNextActions.push(
      "Claim the task on-chain before starting protected execution.",
    );
  }
  if (unreadChatCount > 0) {
    suggestedNextActions.push(
      "Review unread task chat messages from the requester.",
    );
  }
  if (clarifications.some((item) => item.status === "OPEN")) {
    suggestedNextActions.push(
      "Resolve open clarifications or request owner guidance.",
    );
  }
  if (pendingApprovals.length > 0) {
    suggestedNextActions.push(
      "Wait for node-owner approval before continuing the blocked step.",
    );
  }
  if (task.workflow?.deliveryStage === "UNDER_REVIEW") {
    suggestedNextActions.push(
      "Pause execution and wait for requester review of the latest delivery.",
    );
  }
  if (suggestedNextActions.length === 0) {
    suggestedNextActions.push(
      "Continue execution and report progress when a meaningful milestone is reached.",
    );
  }
...
export async function updateWorkerRun(
  ...
  const res = await fetch(
    `${internals(agent).platformUrl}/api/nodes/me/worker-runs/${runId}`,
    {
      method: "PATCH",
      headers: internals(agent).headers(),
...
export async function heartbeatWorkerRun(
  ...
  const res = await fetch(
    `${internals(agent).platformUrl}/api/nodes/me/worker-runs/${runId}/heartbeat`,
    {
      method: "POST",
      headers: internals(agent).headers(),
...
export async function finishWorkerTaskSession(
  agent: AgentPactAgent,
  input: WorkerTaskSessionFinishInput,
): Promise<WorkerRunData> {
  const run = await agent.updateWorkerRun(input.runId, {
    status: input.outcome,
    percent: input.percent ?? (input.outcome === "SUCCEEDED" ? 100 : undefined),
    currentStep:
      input.currentStep ??
...
export async function submitDeliveryForWorkerRun(
  agent: AgentPactAgent,
  ...
  await agent.attachDeliveryTxHash(
    input.taskId,
  ...
  const run = await agent.updateWorkerRun(input.runId, {
    ...
    currentStep:
      input.currentStep ?? "Delivery submitted, waiting for requester review",
    summary:
      input.summary ??
      "Delivery submitted successfully and is now under requester review.",
    metadata: input.metadata,
  });
...
export async function abandonTaskForWorkerRun(
  agent: AgentPactAgent,
...
export async function claimAcceptanceTimeoutForWorkerRun(
  agent: AgentPactAgent,
  ...
  const run = await agent.finishWorkerTaskSession({
    ...
    summary:
      input.summary ??
      "Requester review window expired; acceptance timeout claimed.",
    metadata: input.metadata,
    unwatchTask: input.unwatchTask,
  });
...
export async function gateWorkerRunForApproval(
  agent: AgentPactAgent,
...
export async function executeWorkerRunAction(
  agent: AgentPactAgent,
  runId: string,
  ...
  const res = await fetch(
    `${internals(agent).platformUrl}/api/nodes/me/worker-runs/${runId}/actions`,
    {
      method: "POST",
      headers: internals(agent).headers(),
...
export async function resolveStaleWorkerRuns(
  agent: AgentPactAgent,
  input: ResolveStaleWorkerRunsInput,
): Promise<ResolveStaleWorkerRunsResult> {
  const res = await fetch(
    `${internals(agent).platformUrl}/api/nodes/me/worker-runs/resolve-stale`,
    {
      method: "POST",
      headers: internals(agent).headers(),
      body: JSON.stringify(input),
...
export async function resumeWorkerRunAfterApproval(
  agent: AgentPactAgent,
  ...
  if (!approval) {
    throw new Error(
      `Approval ${input.approvalId} not found for task ${input.taskId}`,
    );
  }
  ...
  if (approval.status !== "APPROVED") {
    throw new Error(
      `Approval ${input.approvalId} resolved with status ${approval.status} and cannot resume the worker`,
    );
  }
  ...
  const run = await agent.updateWorkerRun(input.runId, {
    ...
    percent: input.percent,
    currentStep:
      input.currentStep ?? "Owner approval resolved, execution resumed",
    summary:
      input.summary ??
...
export async function waitForRequesterReviewOutcome(
  agent: AgentPactAgent,
...
export async function syncWorkerRunWithRequesterReview(
  agent: AgentPactAgent,
  ...
  if (input.outcome === "REVISION_REQUESTED") {
    const run = await agent.updateWorkerRun(input.runId, {
      ...
      summary:
        input.summary ?? "Requester review requested another revision pass.",
      metadata: input.metadata,
    });
