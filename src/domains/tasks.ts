import type { TaskTimelineItem, TaskDetailsData, TaskListItem } from "../types.js";
...
export async function confirmTask(agent: AgentPactAgent, escrowId: bigint): Promise<string> {
        return agent.client.confirmTask(escrowId);
    }

export async function declineTask(agent: AgentPactAgent, escrowId: bigint): Promise<string> {
        return agent.client.declineTask(escrowId);
    }

export function getAssignmentSignature(agent: AgentPactAgent, taskId: string): AssignmentSignatureData | undefined {
        return agent.assignmentSignatures.get(taskId);
    }

export async function claimAssignedTask(agent: AgentPactAgent, taskId: string): Promise<string> {
        let assignment = agent.assignmentSignatures.get(taskId);

        if (!assignment) {
            const res = await fetch(`${agent.platformUrl}/api/escrow/assignment/${taskId}`, {
                method: "GET",
            ...
            agent.assignmentSignatures.set(taskId, assignment);
        }
        ...
        agent.assignmentSignatures.delete(taskId);
        console.error(`[Agent] Task claimed on-chain: ${txHash} for task ${taskId}`);
        return txHash;
    }

export async function createTaskDelivery(agent: AgentPactAgent, taskId: string, payload: {
            deliveryHash: string;
            content: string;
            artifacts?: unknown;
        ...
        const res = await fetch(
            `${agent.platformUrl}/api/tasks/${taskId}/deliveries`,
            {
                method: "POST",
                headers: agent.headers(),
...
export async function attachDeliveryTxHash(agent: AgentPactAgent, taskId: string, deliveryId: string, txHash: string): Promise<unknown> {
        const res = await fetch(
            `${agent.platformUrl}/api/tasks/${taskId}/deliveries/${deliveryId}/submit`,
            {
                method: "POST",
                headers: agent.headers(),
...
export async function submitDelivery(agent: AgentPactAgent, escrowId: bigint, deliveryHash: string): Promise<string> {
...
export async function abandonTask(agent: AgentPactAgent, escrowId: bigint): Promise<string> {
        const txHash = await agent.client.abandonTask(escrowId);
        console.error(`[Agent] Task abandoned on-chain: ${txHash}`);
        return txHash;
    }

export async function reportProgress(agent: AgentPactAgent, taskId: string, percent: number, description: string): Promise<void> {
        const res = await fetch(
            `${agent.platformUrl}/api/tasks/${taskId}/progress`,
            {
                method: "POST",
                headers: agent.headers(),
...
export async function claimAcceptanceTimeout(agent: AgentPactAgent, escrowId: bigint): Promise<string> {
        const txHash = await agent.client.claimAcceptanceTimeout(escrowId);
        console.error(`[Agent] Acceptance timeout claimed: ${txHash}`);
        return txHash;
    }

export async function claimDeliveryTimeout(agent: AgentPactAgent, escrowId: bigint): Promise<string> {
        const txHash = await agent.client.claimDeliveryTimeout(escrowId);
        console.error(`[Agent] Delivery timeout claimed: ${txHash}`);
        return txHash;
    }

export async function claimConfirmationTimeout(agent: AgentPactAgent, escrowId: bigint): Promise<string> {
        return agent.client.claimConfirmationTimeout(escrowId);
    }

export async function getRevisionDetails(agent: AgentPactAgent, taskId: string, revision?: number): Promise<unknown> {
        const params = revision ? `?revision=${revision}` : "";
        const res = await fetch(
            `${agent.platformUrl}/api/revisions/${taskId}${params}`,
            { headers: agent.headers() }
        );
...
export async function getTaskTimeline(agent: AgentPactAgent, taskId: string): Promise<TaskTimelineItem[]> {
        const res = await fetch(
            `${agent.platformUrl}/api/tasks/${taskId}/timeline`,
            { headers: agent.headers() }
        );
...
export async function fetchTaskDetails(agent: AgentPactAgent, taskId: string): Promise<TaskDetailsData> {
        const res = await fetch(
            `${agent.platformUrl}/api/tasks/${taskId}/details`,
            { headers: agent.headers() }
        );
...
export async function executeTaskAction(agent: AgentPactAgent, taskId: string, action: TaskAction, note?: string): Promise<TaskActionResult> {
        const res = await fetch(`${agent.platformUrl}/api/nodes/me/tasks/${taskId}/actions`, {
            method: "POST",
            headers: agent.headers(),
...
export async function getAvailableTasks(agent: AgentPactAgent, options: {
        limit?: number;
        offset?: number;
        status?: string;
        ...
        const fetchFromPlatform = async () => {
            const res = await fetch(
                `${agent.platformUrl}/api/tasks?${params}`,
                { headers: agent.headers() }
            );
...
export async function getMyTasks(agent: AgentPactAgent, options: GetMyTasksOptions = {}): Promise<TaskListItem[]> {
        ...
        const res = await fetch(`${agent.platformUrl}/api/tasks?${params.toString()}`, {
            headers: agent.headers(),
        });
...
export async function bidOnTask(agent: AgentPactAgent, taskId: string, message?: string): Promise<unknown> {
        const res = await fetch(
            `${agent.platformUrl}/api/matching/bid`,
            {
                method: "POST",
                headers: agent.headers(),
...
export async function rejectInvitation(agent: AgentPactAgent, taskId: string, reason?: string): Promise<void> {
        const res = await fetch(
            `${agent.platformUrl}/api/matching/reject-invitation`,
            {
                method: "POST",
                headers: agent.headers(),
