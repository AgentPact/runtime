import type { AgentWalletOverview, TokenBalanceInfo, TransactionReceiptSummary, TransactionStatusSummary, GasQuoteRequest, GasQuoteSummary, PreflightCheckRequest, PreflightCheckResult } from "../types.js";
import type {
  AgentCreateOptions,
  AgentConfig,
  TaskEvent,
...
export async function getWalletOverview(agent: AgentPactAgent): Promise<AgentWalletOverview> {
...
export async function getTokenBalanceInfo(agent: AgentPactAgent, token: `0x${string}`): Promise<TokenBalanceInfo> {
...
export async function getTokenAllowance(agent: AgentPactAgent, token: `0x${string}`, spender: `0x${string}`): Promise<bigint> {
        return agent.client.getTokenAllowance(token, agent.walletAddress, spender);
    }

export async function approveToken(agent: AgentPactAgent, token: `0x${string}`, spender: `0x${string}`, amount?: bigint): Promise<string> {
...
export async function waitForTransaction(agent: AgentPactAgent, hash: `0x${string}`, options?: {
...
export async function getTransactionStatus(agent: AgentPactAgent, hash: `0x${string}`): Promise<TransactionStatusSummary> {
        return agent.client.getTransactionStatus(hash);
    }

export async function getGasQuote(agent: AgentPactAgent, params: GasQuoteRequest): Promise<GasQuoteSummary> {
        return agent.client.getGasQuote(params);
    }

export async function preflightCheck(agent: AgentPactAgent, params: PreflightCheckRequest = {}): Promise<PreflightCheckResult> {
        const notes: string[] = [];
